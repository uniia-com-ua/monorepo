...
export default abstract class BaseStrapiClient {
  ...
  public async fetchAPI(
    ...
    if (text) {
      ...
      console.error("[StrapiBaseClient] Strapi API request error:", appError);
  ...
  public getStrapiApiPathByUId(uid: keyof typeof API_ENDPOINTS): string {
    ...
    throw new Error(
      `Endpoint for UID "${uid}" not found. Configure enpoints in lib/strapi-api/client-config.ts`,
    );
  }
}
